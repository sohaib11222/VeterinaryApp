    ...
    <ScreenContainer scroll padded>
      ...
      <Card style={styles.summaryCard}>
        <View style={styles.summaryRow}>
          ...
          <View style={styles.summaryRight}>
            ...
            <Text style={styles.paymentStatus}>Payment: {paymentStatus}</Text>
          </View>
        </View>
      </Card>
      ...
      {paymentStatus === 'UNPAID' && !shippingSet && (
        <Card style={[styles.alert, styles.alertInfo]}>
          <Text style={styles.alertText}>Waiting for pharmacy owner to set shipping fee. You will be able to pay once the shipping fee is set.</Text>
        </Card>
      )}
      {paymentStatus === 'UNPAID' && shippingSet && (
        <Card style={[styles.alert, styles.alertWarning]}>
          <Text style={styles.alertText}>Shipping fee has been set. Please complete payment to confirm your order.</Text>
        </Card>
      )}
      {paymentStatus === 'PAID' && (
        <Card style={[styles.alert, styles.alertSuccess]}>
          <Text style={styles.alertText}>Payment completed. Your order is being processed.</Text>
        </Card>
      )}
      ...
      {(canPay || canCancel) && (
        <Card>
          <View style={styles.actionsRow}>
            {canPay && (
              <Button
                title={payMutation.isPending ? 'Processing...' : `Pay €${total.toFixed(2)}`}
                onPress={handlePay}
                disabled={payMutation.isPending}
              />
            )}
            {canCancel && (
              <Button title="Cancel Order" onPress={() => setShowCancelConfirm(true)} />
            )}
          </View>
        </Card>
      )}
      ...
      <Modal visible={showCancelConfirm} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalBody}>Are you sure you want to cancel order #{orderNumber}?</Text>
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.modalSecondaryBtn} onPress={() => setShowCancelConfirm(false)}>
                <Text style={styles.modalSecondaryBtnText}>No, Keep Order</Text>
              </TouchableOpacity>
              <TouchableOpacity
              ...
              >
                <Text style={styles.modalCancelBtnText}>{cancelMutation.isPending ? 'Cancelling...' : 'Yes, Cancel Order'}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
