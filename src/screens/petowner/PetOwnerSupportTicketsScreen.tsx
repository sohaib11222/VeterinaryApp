...
export function PetOwnerSupportTicketsScreen() {
  ...
  return <ScreenContainer padded>
    ...
    <ResponsiveFilterChips width={148} height={46} lineCount={2} bottomSpacing={spacing.xs} value={status} onChange={setStatus} accessibilityLabel="Filter support tickets by status" options={['', ...SUPPORT_STATUSES].map((value) => ({ value, label: value ? supportLabel(value) : 'All statuses' }))} />
    <Text style={styles.filterLabel}>ISSUE TYPE</Text>
    <ResponsiveFilterChips width={174} height={46} lineCount={2} bottomSpacing={spacing.sm} value={category} onChange={setCategory} accessibilityLabel="Filter support tickets by category" options={[{ value: '', label: 'All categories' }, ...SUPPORT_CATEGORIES.map(([value, label]) => ({ value, label }))]} />
