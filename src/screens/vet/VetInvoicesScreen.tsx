...
export function VetInvoicesScreen() {
  ...
  return (
    <ScreenContainer padded>
      ...
      <View style={styles.filterHeader}><Text style={styles.filterTitle}>{t('vetInvoices.filters.status')}</Text>{(search || status || range !== 'ALL') ? <TouchableOpacity style={styles.resetButton} onPress={resetFilters}><Ionicons name="refresh-outline" size={14} color={colors.primary} /><Text style={styles.resetText}>{t('vetInvoices.filters.reset')}</Text></TouchableOpacity> : null}</View>
      <ResponsiveFilterChips width={118} value={status} onChange={(value) => { setStatus(value as '' | InvoiceStatus); setPage(1); }} accessibilityLabel="Filter invoices by status" options={(['', 'SUCCESS', 'PENDING', 'FAILED', 'REFUNDED'] as const).map((value) => ({ value, label: value ? statusLabel(value) : t('common.all') }))} />
      <Text style={[styles.filterTitle, styles.periodTitle]}>{t('vetInvoices.filters.period')}</Text>
      <ResponsiveFilterChips width={124} value={range} onChange={(value) => { setRange(value as DateRange); setPage(1); }} accessibilityLabel="Filter invoices by date range" options={(['ALL', '30_DAYS', '90_DAYS', 'THIS_YEAR'] as const).map((value) => ({ value, label: t(`vetInvoices.ranges.${value}`) }))} />
...
const styles = StyleSheet.create({
  ...
  searchContainer: { flexDirection: 'row', alignItems: 'center', minHeight: 50, paddingHorizontal: spacing.md, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.background, borderRadius: 14, marginBottom: spacing.md }, searchIcon: { marginRight: spacing.sm }, searchInput: { flex: 1, ...typography.bodySmall, color: colors.text, padding: 0 }, filterHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, filterTitle: { ...typography.caption, color: colors.textSecondary, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 0.5 }, resetButton: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingVertical: 4 }, resetText: { ...typography.caption, color: colors.primary, fontWeight: '800' }, periodTitle: { marginTop: spacing.sm },
